...
def setup_tokenizer(train_config, model_config, **kwargs):
    ...
    if "vallex" in model_config.llm_name.lower():
        ...
        if model_config.phn_tokenizer != None:
            logger.info("loading phn tokenizer from: {}".format(model_config.phn_tokenizer))
            tokenizer = AutoTokenizer.from_pretrained(model_config.phn_tokenizer)
        else:
            tokenizer = AutoTokenizer.from_pretrained(model_config.llm_path)
    ...
    if (train_config.enable_fsdp or train_config.enable_ddp) and train_config.low_cpu_fsdp:
        ...
        if "vallex" in model_config.llm_name.lower():
            ...
            if model_config.random_init is True:
                ...
                llmmodel_config = AutoConfig.from_pretrained(model_config.llm_path)
                model = AutoModelForCausalLM.from_config(llmmodel_config,)
            else:
                model = AutoModelForCausalLM.from_pretrained(
                    model_config.llm_path,
                    load_in_8bit=True if train_config.quantization else None,
